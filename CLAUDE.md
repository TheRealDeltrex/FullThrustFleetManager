...
- Quick reference: `data/quickref/<ruleset>.json`, generated by `tools/extract_quickref.py`
  ...
  book's own wording with a printed page reference, which PLAN 2.5 requires: **never paraphrase
  it by hand.** `Ruleset.quickref()` filters it to the systems present plus a few always-on
